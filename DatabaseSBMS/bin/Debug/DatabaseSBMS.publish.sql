...
/****** Object:  Database [SBMS_DB]    Script Date: 2023-03-30 11:09:17 AM ******/
CREATE DATABASE [SBMS_DB]
 CONTAINMENT = NONE
 ON  PRIMARY 
( NAME = N'SBMS_DB', FILENAME = N'C:\Program Files\Microsoft SQL Server\MSSQL16.MSSQLSERVER\MSSQL\DATA\SBMS_DB.mdf' , SIZE = 8192KB , MAXSIZE = UNLIMITED, FILEGROWTH = 65536KB )
 LOG ON 
( NAME = N'SBMS_DB_log', FILENAME = N'C:\Program Files\Microsoft SQL Server\MSSQL16.MSSQLSERVER\MSSQL\DATA\SBMS_DB_log.ldf' , SIZE = 8192KB , MAXSIZE = 2048GB , FILEGROWTH = 65536KB )
...
INSERT INTO [dbo].[Permission] ([dbo].[Permission].name)VALUES('SaleStuff');
INSERT INTO [dbo].[Permission] ([dbo].[Permission].name)VALUES('PurchaesStuff');
CREATE TABLE [dbo].[Users]
(
	[Id] INT NOT NULL PRIMARY KEY IDENTITY(1,1), 
    [name] NVARCHAR(10) NOT NULL, 
...
DELETE FROM [dbo].SalesItems WHERE [dbo].SalesItems.saleId=@id
DELETE FROM [dbo].SalesDates WHERE [dbo].SalesDates.saleId=@id
End
GO
...
End
GO
PRINT N'Creating [dbo].[NewEmployee]...';
...
INNER JOIN [dbo].[Gender] ON [dbo].[Customers].genId = [dbo].[Gender].Id
INNER JOIN [dbo].[CustCategory] ON [dbo].[Customers].cateId = [dbo].[CustCategory].Id
WHERE [dbo].[Customers].[Id] = @id OR [dbo].[Customers].[name] LIKE @name+'%'
End
GO
...
INNER JOIN [dbo].JobTitle ON [dbo].Employees.jobTitleId = [dbo].JobTitle.Id
INNER JOIN [dbo].Gender ON [dbo].Employees.genderId = [dbo].Gender.Id
WHERE [dbo].[Employees].[Id] = @id OR [dbo].[Employees].[name] LIKE @name+'%' OR [dbo].[Employees].[name] LIKE '%'+@name
End
GO
...
FROM [dbo].[Suppliers]
INNER JOIN [dbo].[SupCategory] ON [dbo].[Suppliers].cateId = [dbo].[SupCategory].Id
WHERE [dbo].[Suppliers].[Id] = @id OR [dbo].[Suppliers].[name] LIKE @name+'%'
End
GO
...
UPDATE [dbo].[JobTitle] SET
			...
			[dbo].[JobTitle].[description] = @descrip
